/* ORNZA Admin — shared sidebar navigation */
(function () {
  ...
  let navHtml = '';
  items.forEach((group) => {
    navHtml += `<div class="portal-nav-label">${group.section}</div>`;
    group.links.forEach((link) => {
      const active = link.id === page ? ' active' : '';
      const badge = link.badge ? `<span class="nav-badge">${link.badge}</span>` : '';
      navHtml += `<a href="${link.href}" class="portal-nav-link${active}">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">${link.icon}</svg>
        ${link.label}${badge}
      </a>`;
    });
  });
  ...
  mount.innerHTML = `
    <div class="portal-brand">
      <div class="portal-brand-logo">ORN<span>Z</span>A</div>
      <span class="portal-brand-badge">Admin</span>
    </div>
    <nav class="portal-nav">${navHtml}</nav>
    <div class="portal-sidebar-footer">
      <div class="portal-user-mini">
        <div class="portal-user-avatar">AK</div>
        <div class="portal-user-info">
          <div class="portal-user-name">Admin Kumar</div>
          <div class="portal-user-role">Store Owner</div>
        </div>
      </div>
      ...
      <a href="login.html" class="portal-back-link" style="margin-top:4px;">
