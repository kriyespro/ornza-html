/* ==========================================
   ORNZA - WEB APP INTERACTIVE LOGIC
   ========================================== */

document.addEventListener('DOMContentLoaded', () => {

  /* -------------------------------------------
     1. Twinkling Starfield Generator
     ------------------------------------------- */
  ...
  if (starsEl) {
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      const size = randomVal < 0.1 ? 2.5 : randomVal < 0.35 ? 1.5 : 1;
  ...
  /* -------------------------------------------
     2. Persistent Launch Countdown
     ------------------------------------------- */
  let launchDate;
  const cachedLaunch = localStorage.getItem('ornza_launch_target');

  if (cachedLaunch) {
    launchDate = new Date(parseInt(cachedLaunch, 10));
  } else {
    launchDate = new Date();
    launchDate.setDate(launchDate.getDate() + 30);
    localStorage.setItem('ornza_launch_target', launchDate.getTime().toString());
  }

  const daysVal = document.getElementById('cd-d');
  const hoursVal = document.getElementById('cd-h');
  const minutesVal = document.getElementById('cd-m');
  const secondsVal = document.getElementById('cd-s');
  ...
  // Set active class on menu links based on current path
  ...
  /* -------------------------------------------
     4. Newsletter lead capture validation
     ------------------------------------------- */
  ...
  if (form && emailInput && messageEl && submitBtn) {
    form.addEventListener('submit', (e) => {
      ...
      const originalText = btnText ? btnText.textContent : 'Notify Me';
      ...
      setTimeout(() => {
        ...
        messageEl.textContent = 'Success! You have been registered for updates.';
        
        setTimeout(() => {
          if (messageEl.textContent.includes('registered')) {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      const fb = document.getElementById('contact-feedback-message');

      if (!name || !email || !subject || !message) {
        fb.className = "form-message error";
      ...
      const sbtn = contactForm.querySelector('.form-submit-btn');
      sbtn.disabled = true;
      const originalBtnText = sbtn.textContent;
      sbtn.textContent = "Sending Inquiry...";

      setTimeout(() => {
        sbtn.disabled = false;
        sbtn.textContent = originalBtnText;
        contactForm.reset();
        fb.className = "form-message success";
        fb.textContent = "Inquiry sent successfully! Our showroom representative will reach out shortly.";
        
        setTimeout(() => {
          fb.textContent = "";
          fb.className = "form-message";
        }, 6000);
      }, 1200);
    });
  }
  ...
  /* -------------------------------------------
     7. Comprehensive Product Database
     ------------------------------------------- */
  const categoryData = {
    rings: {
      title: "The Aurelia Rings",
      desc: "Hand-sculpted in 18k gold vermeil, our upcoming ring collection features intricate filigree work and brilliant laboratory-grown solitaires.",
      material: "18K Gold Vermeil",
      stones: "VVS1 Moissanite Solitaire",
      image: "assets/ring_preview.png"
    },
    diamonds: {
      title: "Celestial Diamonds",
      desc: "Flawless brilliance, conflict-free, and ethically crafted. Our signature diamond collection features high-refraction gems precision-cut.",
      material: "925 Sterling Silver base",
      stones: "Superior Grade Cubic Zirconia",
      image: "assets/diamond_preview.png"
    },
    necklaces: {
      title: "Empress Neckpieces",
      desc: "Grace your neckline with fluid gold chains and stunning centerpieces. Drawing inspiration from royal Indian heritage.",
      material: "22K Gold Plated Brass",
      stones: "Polki & Premium CZ accents",
      image: "assets/necklace_preview.png"
    },
    bangles: {
      title: "Vaikunth Bangles",
      desc: "Intricately detailed bangles with traditional patterns and modern closures. Crafted to sit comfortably on your wrist.",
      material: "Anti-tarnish Gold Alloy",
      stones: "Micro-paved Swiss Zirconia",
      image: "assets/bangle_preview.png"
    },
    earrings: {
      title: "Aria Drop Earrings",
      desc: "From delicate studs to dramatic chandelier drops. Lightweight yet visually striking earrings designed to frame your face.",
      material: "Hypoallergenic Gold Plating",
      stones: "Freshwater Pearls & CZ",
      image: "assets/earring_preview.png"
    },
    bracelets: {
      title: "Minimalist Chains",
      desc: "Delicate, stackable bracelets that redefine everyday luxury. A fusion of clean contemporary lines and timeless gold charms.",
  ...
  /* -------------------------------------------
     8. Shared Glassmorphic Preview Modal Setup
     ------------------------------------------- */
  ...
  const modalMaterial = document.getElementById('modal-attr-material');
  const modalStones = document.getElementById('modal-attr-stones');
  const modalWhatsapp = document.getElementById('modal-whatsapp-link');

  let modalAddCartBtn = document.getElementById('modal-add-cart-btn');
  if (!modalAddCartBtn && modal && modalWhatsapp) {
    modalAddCartBtn = document.createElement('button');
    modalAddCartBtn.type = 'button';
    modalAddCartBtn.id = 'modal-add-cart-btn';
    modalAddCartBtn.className = 'modal-add-cart-btn';
    modalAddCartBtn.textContent = 'Add to Cart';
    modalWhatsapp.parentNode.insertBefore(modalAddCartBtn, modalWhatsapp);
  }

  const openPreview = (title, desc, material, stones, image, prodId) => {
    if (!modal) return;
    ...
    if (modalStones) modalStones.textContent = stones;
    
    if (modalWhatsapp) {
      const baseText = `Hi ORNZA! I saw the pre-launch preview of "${title}" (${material}) and would love to get more details and pricing when you launch.`;
      modalWhatsapp.href = `https://wa.me/919999999999?text=${encodeURIComponent(baseText)}`;
    }

    if (modalAddCartBtn) {
      modalAddCartBtn.onclick = () => {
        const product = prodId && productsData[prodId];
        const price = product?.price || '₹0';
        const img = product?.image || image;
        const name = product?.title || title;
        if (window.OrnzaCart) {
          window.OrnzaCart.addToCart(name, price, img, modalAddCartBtn);
        }
      };
    }
  ...
  /* -------------------------------------------
     9. Setup Dynamic WhatsApp Links & Quick View Actions
     ------------------------------------------- */
  
  // Setup links for catalog inquiry buttons
  document.querySelectorAll('.inquire-btn').forEach(btn => {
    const prodId = btn.getAttribute('data-product-id');
    const product = productsData[prodId];
    if (product) {
      const baseText = `Hi ORNZA! I would like to inquire about the piece "${product.title}" (${product.material}). Can you share price and availability?`;
      btn.href = `https://wa.me/919999999999?text=${encodeURIComponent(baseText)}`;
      btn.target = "_blank";
      btn.rel = "noopener noreferrer";
    }
  });

  // Setup links for "Buy Now" buttons (WhatsApp intent)
  document.querySelectorAll('.buy-now-btn').forEach(btn => {
    const prodId = btn.getAttribute('data-product-id');
    const product = productsData[prodId];
    if (product) {
      const baseText = `Hi ORNZA! I want to BUY NOW: "${product.title}" (${product.material}). Please confirm total price and delivery timeline.`;
      btn.href = `https://wa.me/919999999999?text=${encodeURIComponent(baseText)}`;
      btn.target = "_blank";
      btn.rel = "noopener noreferrer";
    }
  ...
  if (window.OrnzaCart) {
    window.OrnzaCart.bindProductPageButtons?.();
  }

  // Setup click listener for product Quick View buttons
  document.querySelectorAll('.quick-view-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const prodId = btn.getAttribute('data-product-id');
      const product = productsData[prodId];
      if (product) {
        openPreview(product.title, product.desc, product.material, product.stones, product.image, prodId);
      }
    });
  });

  // Setup click listener for Home page category preview cards (teaser triggers)
  document.querySelectorAll('.jewel-item').forEach(item => {
    item.addEventListener('click', () => {
      const cat = item.getAttribute('data-category');
      const data = categoryData[cat];
      if (data) {
        openPreview(data.title, data.desc, data.material, data.stones, data.image);
      }
    });
  });
});
